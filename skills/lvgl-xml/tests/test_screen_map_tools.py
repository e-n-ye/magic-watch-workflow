...
FIXTURE = json.loads((Path(__file__).resolve().parents[1] / "examples" / "watchface-screen-map.json").read_text(encoding="utf-8"))
...
class ScreenMapToolsTests(unittest.TestCase):
    ...
    def test_nested_coordinates_are_checked_against_parent(self):
        ...
        next(item for item in document["objects"] if item["name"] == "watchface_summary_hint")["x"] = 180
        errors = validate(document)
        self.assertTrue(any("watchface_summary_hint" in error and "exceeds parent" in error for error in errors))

    def test_sibling_overlap_is_rejected(self):
        document = copy.deepcopy(FIXTURE)
        document["objects"][3]["x"] = 100
        errors = validate(document)
    ...
    def test_generator_emits_only_supported_nodes(self):
        ...
        self.assertIn('clickable="true"', xml)
        self.assertNotIn("lv_image", xml)
