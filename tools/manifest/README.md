...
The VS Code `F411: Pack Signed App (host)` task reads
`F411_SIGNING_KEY`, `F411_FIRMWARE_VERSION`, and `F411_SECURITY_COUNTER` from
the machine-local environment. The private key must match the public key
compiled into the Bootloader; generating an unrelated key will produce a
package that the board correctly rejects.
