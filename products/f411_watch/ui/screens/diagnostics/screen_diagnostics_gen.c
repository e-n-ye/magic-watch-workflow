...
lv_obj_t * screen_diagnostics_create(void)
{
    ...
    static lv_style_t style_diagnostics;

    static bool style_inited = false;

    if (!style_inited) {
        /*Init all styles*/
        lv_style_init(&style_diagnostics);

        lv_style_set_bg_opa(&style_diagnostics, (255 * 100 / 100));
        lv_style_set_bg_color(&style_diagnostics, lv_color_hex(0x101820));
    ...
    if (f411_watch_ui_check_target(F411_WATCH_UI_TARGET_ALL)) {
        ...
        lv_obj_add_style(lv_obj_0, &style_diagnostics, 0);
        ...
        lv_label_set_text(lv_label_1, "DIAGNOSTICS");
